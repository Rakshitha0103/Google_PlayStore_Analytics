...
translation_map = {'Beauty':'सौंदर्य','Business':'வியாபாரம்','Dating':'Dating_DE'}
...
agg_df = df.groupby(['YearMonth','Category_Translated'])['Installs'].sum().reset_index()
...
if 18 <= current_hour_ist <= 21:
    highlight_growth = st.checkbox("Highlight >20% month-over-month growth", value=True)
    fig = px.line(agg_df, x='YearMonth', y='Installs', color='Category_Translated', markers=True,
        ...
        for cat in agg_df['Category_Translated'].unique():
            ...
            for i in range(1,len(data)):
                if data['Pct_Change'].iloc[i] > 0.2:
                    fig.add_vrect(x0=data['YearMonth'].iloc[i-1], x1=data['YearMonth'].iloc[i],
                                  fillcolor="LightSalmon", opacity=0.3, layer="below", line_width=0)
    fig.update_traces(hovertemplate="<b>%{fullData.name}</b><br>Month: %{x|%Y-%m}<br>Total Installs: %{y:,}<br>MoM Growth: %{customdata:.1%}", customdata=agg_df['Pct_Change'])
    fig.update_layout(xaxis=dict(dtick="M1", tickformat="%Y-%m"), hovermode="x unified", legend_title="App Category")
